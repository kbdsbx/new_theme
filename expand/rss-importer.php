...
class RSS_Import extends WP_Importer {
	...
	function get_posts() {
		...
		foreach ($this->posts as $post) {
            ...
            $thumbnail = preg_replace( '|/uploads(.*?)|is', wp_upload_dir()['baseurl'] . '$1', esc_sql(trim($thumbnail[1])) ); 
            ...
            preg_match( '|<tags>(.*?)</tags>|is', $post, $tags_input );
            if ( $tags_input ) {
			...
			$post_author = 1;
			$post_status = 'publish';
			$this->posts[$index] = compact('post_author', 'thumbnail', 'tags_input', 'post_date', 'post_date_gmt', 'post_content', 'post_title', 'post_status', 'guid', 'categories');
			$index++;
		}
	}
	...
	function import_posts() {
		...
		foreach ($this->posts as $post) {
			...
			if ($post_id = post_exists($post_title, $post_content, $post_date)) {
                ...
                if ('' != $thumbnail){
                    $attachment_id = wp_insert_attachment( array(), $thumbnail, $post_id );
                    if ( 0 != $attachment_id ) {
